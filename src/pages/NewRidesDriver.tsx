  ...
  useEffect(() => {
    if (userRole === "DRIVER") {
      const fetchRides = async () => {
        const data = await props.driverService.GetDriverStatus(userMail);
  ...
  useEffect(() => {
    ...
    if (isRideActive) {
      interval = setInterval(() => {
        if (arrivalTimeRef.current !== null && arrivalTimeRef.current > 0) {
          ...
          {rideData.map((ride) => (
            <tr className={styles.row} key={ride.createdAtTimestamp}>
              ...
              <td className={styles.dataCell}>
                <button
                  onClick={() =>
                    handleAcceptRide(ride.clientEmail, ride.createdAtTimestamp)
                  }
                  disabled={
